                   ...
                   int sockindex)
{
  ...
  if(SSL_SET_OPTION(issuercert)) {
    ...
    if (conn->floadcertfile) {
      issuerp = *((gnutls_datum_t *)conn->floadcertfile(SSL_SET_OPTION(issuercert), conn->loadcertfile_client));
    } else {
    ...
    if (conn->funloadfile) {
      conn->funloadfile(&issuerp, conn->unloadfile_client);
    } else {
      unload_file(issuerp);
    }
    if(rc <= 0) {
