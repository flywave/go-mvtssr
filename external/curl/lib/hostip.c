                ...
                int port)
{
  ...
  if (!dns && conn->fresolverdns) {
    /* fetch dns from user cache  */
    Curl_addrinfo *info = (Curl_addrinfo *)conn->fresolverdns(hostname,
                                                              port,
                                                              conn->resolverdns_client);
    if (info) {
      Curl_addrinfo *ai = NULL;
      Curl_addrinfo *ainext = NULL;
    
      Curl_addrinfo *ca = NULL;
    
      for(ca = info; ca != NULL; ca = ca->ai_next) {
        if (ai == NULL) {
          ainext = ai = Curl_cloneaddrinfo(ca);
        } else {
          ainext->ai_next = Curl_cloneaddrinfo(ca);
          ainext = ainext->ai_next;
        }
      }
    
      dns = Curl_cache_addr(data, ai, hostname, port);
    }
  } else if(dns && (data->set.dns_cache_timeout != -1)) {
